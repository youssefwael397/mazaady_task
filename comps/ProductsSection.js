...
export const ProductsSection = () => {
  ...
  return (
    <div className="px-3">
      <h3 className={`text-[#333333] text-3xl max-xl:text-2xl font-bold my-3`}>
        Products{' '}
        <span className="text-[#828282] text-base font-normal">( 12 )</span>
      </h3>

      {products.map(
        ({ src, title, price, days, hours, minutes, tag, favorite }, id) => (
          <>
            <div className="flex justify-between max-xl:relative mt-8" key={id}>
              <div className="flex items-center max-xl:justify-between max-xl:items-start">
                <div className="xl:relative">
                  <img
                    src={src}
                    width={mediaQuery.matches ? 180 : 140}
                    height={100}
                    className=""
                    alt={title}
                  />
                  {tag == 'Live auction' ? (
                    <div className={styles.tag_image}>{tag}</div>
                  ) : (
                    <div className={`${styles.tag_image} ${styles.hot_sale}`}>
                      {tag}
                    </div>
                  )}
                </div>
                <div className="ml-5 text-lg max-xl:text-base">
                  ...
                  <p className="text-[#828282] mt-3 max-xl:mt-2">
                    Starting price{' '}
                    <span className="text-[#333333] text-xl max-lg:text-base font-semibold">
                      {price}
                    </span>
                  </p>
                  {mediaQuery.matches ? (
